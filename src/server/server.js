...
io.on('connection', (socket) => {
  ...
  socket.on(HORIZON__DRAFTED_CARDS, (chosenCard, passedCards) => {
    ...
    if (everyoneReady) {
      ...
      keptCardsDict = new Map([...keptCardsDict].map(([u, keptCards]) => (
        [u, [...keptCards, chosenCardsDict.get(u)]]
      )));
      chosenCardsDict = new Map(usernames.map((u) => [u, null]));
      ...
      if (isLastRound) {
        usernames.forEach((u) => {
          const lastCard = horizonHands.get(u)[0];
          const keptCards = [...keptCardsDict.get(u), lastCard];
          keptCardsDict = new Map([...keptCardsDict, [u, keptCards]]);
          horizonHands = new Map([...horizonHands, [u, []]]);
        });
      }

      // Send information to clients.
      usernames.forEach((u) => {
        const hand = horizonHands.get(u);
        const keptCards = keptCardsDict.get(u);
        sockets.get(u).emit(HORIZON__UPDATE_HAND, hand);
        sockets.get(u).emit(HORIZON__UPDATE_KEPT_CARDS, keptCards);
      });
